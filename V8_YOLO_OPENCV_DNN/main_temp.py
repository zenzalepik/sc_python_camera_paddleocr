...
class RealTimeDistanceDetector:
    ...
    def __init__(self, camera_id=0, confidence_threshold=0.5):
        ...
        # Load YOLO model (OpenCV DNN)
        print("Loading YOLO model (OpenCV DNN)...")
        
        # Pakai YOLOv3-Tiny (native OpenCV DNN support)
        yolo_config = os.path.join(os.path.dirname(__file__), 'yolov3-tiny.cfg')
        yolo_weights = os.path.join(os.path.dirname(__file__), 'yolov3-tiny.weights')
        ...
        if os.path.exists(yolo_weights) and os.path.exists(yolo_config):
            ...
            print(f"   Please download yolov3-tiny.weights (~34 MB)")
            self.detector = None
        
        # Target classes untuk tracking (class names dari COCO dataset)
        ...
        # Class names (hardcode untuk COCO)
        self.class_names = ['person', 'bicycle', 'car', 'motorcycle', 'bus', 'truck', 'cell phone']
        ...
        # OCR Manager - PaddleOCR v5 Mobile (OPTIONAL - disable untuk hemat CPU)
        # Comment out untuk disable OCR
        # self.ocr_manager = OCRManager(lang='en', conf_threshold=0.5)
        self.ocr_manager = None  # Disabled untuk hemat CPU
    ...
    def start(self):
        ...
        try:
            ...
            # Create window dengan error handling
            window_name = 'Object Distance Detection - OpenCV DNN'
            cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
            cv2.resizeWindow(window_name, frame_width, frame_height)
            print(f"[OK] Window created: {frame_width}x{frame_height}")
    ...
    def detect_objects(self, frame):
        ...
        if len(detections) > 0:
            print(f"[DEBUG] Detected {len(detections)} object(s):")
